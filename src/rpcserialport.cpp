...
RPCSerialPort::RPCSerialPort() {
    QObject::connect(&serial_port, &QSerialPort::readyRead, [this]() {
        if (!currently_in_waitReceived) {
            ...
            qDebug() << "serial_input_thread  waitReceived threadID:" << QThread::currentThreadId();
        }
    });
}
...
bool RPCSerialPort::isConnected() {
   // return serial_port.isOpen();
    return Utility::promised_thread_call(this,  [this] { return serial_port.isOpen(); });
}

bool RPCSerialPort::connect(QString port_name, uint baud) {
    return Utility::promised_thread_call(this,  [this, port_name, baud] {
...
void RPCSerialPort::send(const QByteArray &data, const QByteArray &displayed_data) {
    auto size = serial_port.write(data);
    // QCoreApplication::processEvents();
    qDebug() << "serial_input_thread  sending threadID:" << QThread::currentThreadId();
    if (size == -1) {
        return;
    }
    if (size != data.size()) {
...
void RPCSerialPort::close() {
    // serial_port.close();
    return Utility::promised_thread_call(this, [this] { serial_port.close(); });
}
